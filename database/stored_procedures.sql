...
BEGIN
		...
		get_events: LOOP
			...
			IF Case_before = Case_act THEN 
				SET runtime = TIMESTAMPDIFF(MINUTE, TS_before, TS_act);
...
BEGIN
	...
	ELSE
		...
		INSERT INTO prozessteilschritte (Durchlaufzeit, Haeufigkeit, Aktivitaeten_ID, Aktivitaeten_VG_ID, ProzessvariantenID)
		SELECT durchlaufzeit, 1, aktiv, aktiv_vg, ProcvarID FROM teilschritt_tmp WHERE CaseID = Case_ID ORDER BY ID;
	END IF;
